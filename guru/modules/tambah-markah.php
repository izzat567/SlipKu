...
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] === 'get_students') {
        $class = $_GET['class'] ?? '';
        $rows = is_numeric($class) ? getStudentsByClass((int)$class) : getStudentsByClassName($class);
    ...
    if ($_GET['ajax'] === 'get_marks') {
        ...
        if (!$pep_id && !$kelas_id) {
            $sql = "SELECT m.id, m.id_pelajar, m.markah, m.gred, m.catatan,
                    ...
                    AND (k.id IN (SELECT DISTINCT id_kelas FROM pengajar WHERE id_guru=? AND status='aktif') OR k.id_guru=?)
                    ORDER BY p.nama ASC";
        ...
        if ($stmt) { $stmt->execute(); $marks = $stmt->get_result()->fetch_all(MYSQLI_ASSOC); $stmt->close(); }
        else $marks = [];
...
if (isset($_POST['ajax'])) {
    ...
    if ($_POST['ajax'] === 'add_bulk_marks') {
        ...
        if (!$marks_data) { echo json_encode(['success'=>false,'message'=>'Tiada data']); exit(); }
    ...
    if ($_POST['ajax'] === 'update_mark') {
        ...
        if (!$mark_id) { echo json_encode(['success'=>false,'message'=>'ID tidak sah']); exit(); }
        ...
        if ($stmt->execute()) echo json_encode(['success'=>true,'message'=>'Markah dikemaskini','gred'=>$gred]);
...
.action-btn.success{background:var(--success);color:white}.action-btn.success:hover{background:#0da271}
.action-btn.danger{background:var(--danger);color:white}.action-btn.danger:hover{background:#dc2626}
.action-btn.secondary{background:#e5e7eb;color:var(--dark-gray)}.action-btn.secondary:hover{background:#d1d5db}
    ...
    <div id="tab-tambah" class="tab-content <?php echo $active_tab==='tambah'?'active':''; ?>">
        ...
        <form method="GET" action="" id="filterForm">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input type="number" class="form-input" name="markah_penuh" id="fullMarks" value="<?php echo $markah_penuh; ?>" min="1" max="200" required>
            </div>
            <div class="form-group">
              <label class="form-label required">Markah Lulus</label>
              <input type="number" class="form-input" name="markah_lulus" id="passingMarks" value="<?php echo $markah_lulus; ?>" min="0" max="200" required>
            </div>
          </div>
      ...
      <div class="summary-grid">
        <div class="summary-stat">
          ...
          <div class="stat-value" id="avgMark">0.0</div>
        ...
        <div class="summary-stat">
          ...
          <div class="stat-value" id="highMark">0</div>
        ...
        <div class="summary-stat">
          ...
          <div class="stat-value" id="lowMark">0</div>
        ...
        <div class="summary-stat">
          ...
          <div class="stat-value" id="completedCount">0/<?php echo count($students); ?></div>
      ...
      <div class="actions-bar">
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          ...
          <span id="marksStatusText" style="font-size:12px;color:var(--medium-gray);align-self:center;"><?php echo count($students); ?> pelajar. Belum ada markah.</span>
        </div>
        <button class="btn btn-success" onclick="simpanSemuaMarkah()"><i class="fas fa-save"></i> Simpan Semua Markah</button>
      </div>
      ...
      <div class="section-card" style="margin-bottom:15px;">
        <div class="search-bar">
          <div class="search-wrap">
            ...
            <input type="text" class="search-field" id="kmSearch" placeholder="Cari nama, No. KP, kelas..." oninput="filterKmTable()">
          </div>
...
<script>
// ===================== STATE =====================
const FULL_MARKS   = <?php echo $markah_penuh; ?>;
const PASS_MARKS   = <?php echo $markah_lulus; ?>;
const EXAM_ID      = <?php echo (int)$selected_exam; ?>;
const ALL_STUDENTS = <?php echo json_encode($students); ?>;

let tambahMarks   = {};   // sid -> {markah}
let kmData        = [];   // full list from server
let kmChanges     = {};   // markId -> {markah_baru,catatan}
...
function onMarkInput(sid, val){
  ...
  if(mv!==null && !isNaN(mv)){
    ...
    const pass = mv>=pm;
    se.textContent=pass?'LULUS':'GAGAL'; se.className=`status-badge ${pass?'status-success':'status-danger'}`;
  } else {
...
function validateMI(sid, fm){
  ...
  if(!isNaN(v)&&(v<0||v>fm)){inp.classList.add('out-of-range');toast('Markah mesti antara 0 dan '+fm,'warning');}
...
function updateTambahSummary(){
  ...
  const am=document.getElementById('avgMark'); if(am) am.textContent=avg;
  const hm=document.getElementById('highMark'); if(hm) hm.textContent=high;
  const lm=document.getElementById('lowMark'); if(lm) lm.textContent=low;
  const cc=document.getElementById('completedCount'); if(cc) cc.textContent=`${vals.length}/${total}`;
}
...
function simpanSatu(sid, examId, btn){
  ...
  fetch('tambah-markah.php',{
    ...
    body:new URLSearchParams({ajax:'add_single_mark',id_pelajar:sid,id_peperiksaan:examId,markah:mv,catatan:notes})
...
function simpanSemuaMarkah(){
  ...
  confirm2(`Simpan markah untuk ${keys.length} pelajar?`,()=>{
    const data = keys.map(sid=>({
      id_pelajar:parseInt(sid), id_peperiksaan:EXAM_ID,
    ...
    }).catch(()=>toast('Ralat rangkaian','error'));
  });
}
    ...
    .then(d=>{
      if(d.success){
        kmData = d.marks.map(m=>({
          ...
          studentClass:m.nama_kelas, peperiksaan:m.nama_peperiksaan, subjek:m.nama_subjek,
          originalMark:parseInt(m.markah), currentMark:parseInt(m.markah),
          originalGrade:m.gred||'', catatan:m.catatan||''
        }));
        kmChanges={};
        renderKmTable(kmData);
      } else {
        wrap.innerHTML='<div class="empty-state"><i class="fas fa-exclamation-circle"></i><h3>Gagal memuatkan data</h3></div>';
      }
    })
    .catch(()=>{
...
function renderKmTable(data){
  const wrap = document.getElementById('kmTableWrap');
  if(!data.length){
    wrap.innerHTML='<div class="empty-state"><i class="fas fa-search"></i><h3>Tiada markah dijumpai</h3><p>Tiada data markah. Sila tambah markah pelajar melalui tab <strong>Tambah Markah</strong> terlebih dahulu.</p></div>';
    return;
  }
  ...
  let html=`<table class="marks-table" id="kmTable">
    <thead><tr>
      ...
      <th>MARKAH ASAL</th><th>MARKAH BARU (0–100)</th>
  ...
  data.forEach((m,i)=>{
    ...
    const dispMark = chg?kmChanges[m.id].markah_baru:m.originalMark;
    const dispCat  = chg?kmChanges[m.id].catatan:m.catatan;
    const ng = gradeOf(dispMark,100);
    const passed = dispMark>=40;
    ...
    html+=`<tr class="${chg?'row-changed':''}" id="kmrow-${m.id}"
      ...
      <td>
        <div class="student-row">
          ...
          <div class="student-info">
            <h4>${esc(m.studentName)}</h4>
            <p>${esc(m.studentIC)}</p>
          </div>
        </div>
      </td>
      <td>
        <div style="font-size:12px;font-weight:600;color:var(--dark-gray);">${esc(m.peperiksaan)}</div>
      ...
      <td>
        <input type="number" class="mark-input ${chg?'changed':''}" id="km-${m.id}"
          min="0" max="100" value="${dispMark}"
          oninput="onKmInput(${m.id},this.value)"
          placeholder="0–100">
      </td>
      ...
      <td><span class="status-badge ${passed?'status-success':'status-danger'}" id="km-status-${m.id}">${passed?'LULUS':'GAGAL'}</span></td>
      <td>
        <input type="text" class="notes-input" id="km-notes-${m.id}"
          value="${esc(dispCat)}" placeholder="Catatan..."
          oninput="onKmNotes(${m.id},this.value)">
      </td>
    </tr>`;
  });
...
function onKmInput(mid, val){
  ...
  if(row){
    row.className=chg?'row-changed':'';
    row.dataset.status=pass?'lulus':'gagal';
  }
  updateChangesBadge();
}
...
function updateChangesBadge(){
  ...
  if(el){
    if(cnt>0){el.style.display='inline-block';el.textContent=`${cnt} perubahan`;}
...
function filterKmTable(){
  ...
  rows.forEach(r=>{
    const mn = r.dataset.name||'';
    const mi = r.dataset.ic||'';
    const mk = r.dataset.kelas||'';
    const mp = r.dataset.pep||'';
    const ms = r.dataset.status||'';

    const okS = !s||(mn.includes(s)||mi.includes(s)||mk.includes(s));
    const okP = !pep||mp.includes(pep);
    const okK = !kel||mk.includes(kel);
    let okSta = true;
    if(sta==='lulus') okSta=ms==='lulus';
    else if(sta==='gagal') okSta=ms==='gagal';
    else if(sta==='berubah') okSta=r.classList.contains('row-changed');

    r.style.display=(okS&&okP&&okK&&okSta)?'':'none';
  });
}
...
function gradeOf(m, fm){
  fm=fm||100; const p=(parseInt(m)/fm)*100;
  if(p>=90) return 'A+'; if(p>=80) return 'A'; if(p>=70) return 'B';
  if(p>=60) return 'C'; if(p>=50) return 'D'; if(p>=40) return 'E'; return 'F';
}

function gradeClass(g){
  const map={'A+':'grade-aplus','A':'grade-a','B':'grade-b','C':'grade-c','D':'grade-d','E':'grade-e','F':'grade-f'};
  return map[g]||'grade-f';
}
...
if(menuToggle) menuToggle.addEventListener('click',()=>{ sidebar.classList.toggle('sidebar-active'); sidebarOverlay.classList.toggle('active'); });
if(sidebarOverlay) sidebarOverlay.addEventListener('click',()=>{ sidebar.classList.remove('sidebar-active'); sidebarOverlay.classList.remove('active'); });
...
// Auto-load kemaskini if that tab is active
